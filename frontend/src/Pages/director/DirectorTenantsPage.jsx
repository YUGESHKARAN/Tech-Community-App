...
import { enterImpersonation } from "../../hooks/director/Useimpersonation";
...
import {
  ...
  TbActivity, TbChartBar, TbHistoryToggle, TbUserShield,
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-2 flex-wrap">
        <input type="text" placeholder="Filter by tenant ID..." value={filter.tenantId}
          ...
          className="theme border border-[#1e293b] rounded-lg px-3 py-1.5 text-xs text-gray-200 placeholder-gray-600 focus:outline-none focus:border-white/15 w-44" />
...
const ImpersonationPanel = ({ tenant, onClose, showToast }) => {
  ...
  const handleEnter = () => {
    ...
    try {
      ...
      // in handleEnter inside ImpersonationPanel
      enterImpersonation(token, {
        tenantId:      tenant.tenantId,
        tenantName:    tenant.name,
        jti,
        expiresAt,
        directorEmail: getItem("email"), // decoded director email from encoded storage
      });
      // enterImpersonation triggers window.location.href = "/" so page reloads
    } catch (err) {
      showToast(err.message || "Failed to enter impersonation", "error");
      setEntering(false);
    }
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3 p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
        ...
        <p className="text-xs text-amber-300/80 leading-relaxed">
          Impersonation grants admin-level access to <b>{tenant.name}</b> for 15 minutes.
          All actions are audit-logged. A banner will be shown across all pages
          so you always know you are impersonating.
        </p>
      </div>
      ...
      {!token ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <div className="bg-white/[0.02] border border-[#1e293b] rounded-xl p-3 font-mono text-[9px] text-gray-400 break-all leading-relaxed max-h-20 overflow-y-auto">
              {token}
            </div>
          </div>
          ...
          <p className="text-[9px] text-gray-600 text-center leading-relaxed">
            Clicking "Enter" reloads the platform with this token active.
            A purple banner appears at the top — click "Exit impersonation" to return here.
          </p>
        </div>
      )}
    </div>
...
//  TENANT CARD
// ─────────────────────────────────────────────────────────────────────────────
const TenantCard = ({ tenant, selected, onSelect, onEdit, onDelete, onToggleActive, onViewHealth, onImpersonate, role }) => {

  const canWrite = ["super_director", "director"].includes(role);
  ...
  // ];
  const menuItems = [
    ...(canWrite ? [{ label: "Edit", icon: <TbPencil className="text-sm" />, onClick: () => onEdit(tenant) }] : []),
    { label: "Health & usage", icon: <TbActivity className="text-sm" />, onClick: () => onViewHealth(tenant) },
    ...(canWrite ? [{ label: "Impersonate", icon: <TbUserShield className="text-sm" />, onClick: () => onImpersonate(tenant) }] : []),
    ...(canWrite ? [{
      label: tenant.active ? "Deactivate" : "Activate",
      icon: tenant.active ? <TbToggleLeft className="text-sm" /> : <TbToggleRight className="text-sm text-emerald-400" />,
      onClick: () => onToggleActive(tenant),
    }] : []),
    ...(role === "super_director" ? [{ label: "Delete", icon: <TbTrash className="text-sm" />, onClick: () => onDelete(tenant), danger: true }] : []),
  ];
...
function DirectorTenantsPage() {
  ...
  useEffect(() => {
    if (!DIRECTOR_ROLES.includes(role)) navigate("/", { replace: true });
  }, [role, navigate]);
